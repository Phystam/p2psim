...
#include "tree.hh"
	...
	while(events<MAX_STORY) //eventloop
	{
		...
		P1LL.SetVectM(P1L,Mi);
		P2LL.SetVectM(P2L,Ma);
		TVector3 PAL(0,0,PA);
		PALL.SetVectM(PAL,MA);
		PBLL.SetVectM(PB,MB);
		TLorentzVector target(0,0,0,Ma);
		MissingMass=PALL+target-P1LL-P2LL;
		ReconstructedEx=MissingMass.M()-MBgs;
		theta_1 = P1L.Theta();		
		theta_2 = P2L.Theta();		
		theta_B = PB.Theta();		
